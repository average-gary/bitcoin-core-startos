...
export const main = sdk.setupMain(async ({ effects, started }) => {
  ...
  bitcoinArgs.push('-chain=testnet4')
  ...
  // Add network-specific settings as command-line arguments for testnet4
  // (these can't be in bitcoin.conf main section when using -chain=testnet4)
  bitcoinArgs.push(`-rpcbind=${conf.rpcbind}`)
  bitcoinArgs.push(`-rpcallowip=${conf.rpcallowip}`)
      ...
      ready: {
        ...
        fn: async () => {
          const res = await bitcoindSub.exec([
            'bitcoin-cli',
            `-conf=${rootDir}/bitcoin.conf`,
            `-rpccookiefile=${rootDir}/testnet4/${bitcoinConfDefaults.rpccookiefile}`,
            `-rpcconnect=${conf.rpcbind}`,
            'getblockchaininfo',
          ])
  ...
  if (conf.prune) {
    await configToml.write(effects, {
      ...
      bind_port: rpcPort,
      cookie_file: `${rootDir}/testnet4/${bitcoinConfDefaults.rpccookiefile}`,
      tor_proxy: `${osIp}:9050`,
      tor_only: conf.onlynet ? conf.onlynet.includes('onion') : false,
      passthrough_rpcauth: `${rootDir}/bitcoin.conf`,
      passthrough_rpccookie: `${rootDir}/testnet4/${bitcoinConfDefaults.rpccookiefile}`,
    })
